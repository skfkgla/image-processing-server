...
**폴링 방식**: Mock Worker가 Webhook을 지원하지 않으므로 완료 감지는 서버 내부 폴링으로 처리합니다. Mock Worker는 GPU 기반 무거운 AI 추론 작업으로 응답 시간이 수 초~수십 초까지 변동되며 순간 트래픽에 취약합니다. 비동기 동시 폴링 시 부하가 집중되어 429 및 불안정한 응답을 유발할 수 있어 순차 처리로 부하를 제어합니다.

**이미지를 URL로 전달받는 이유**: 대용량 이미지를 서버가 직접 수신하면 메모리와 네트워크 부하를 서버가 감당해야 합니다. 클라이언트가 S3 같은 스토리지에 먼저 업로드한 뒤 URL만 전달하면 이미지 전송 부하가 서버를 거치지 않으므로, 서버는 URL만 받아 Worker에 전달하는 가벼운 역할을 유지할 수 있습니다.
...
| 폴링 방식 | 동기(순차) | Mock Worker의 특성상 동시 요청 집중 시 불안정 — 순차 처리로 부하 제어 (상세 내용은 핵심 설계 결정 참고) |
...
**순차 → 병렬 전환 고려 시점**: 1회 폴링 사이클이 원하는 퍼포먼스 기준을 충족하지 못할 경우 순차 처리의 한계로 판단합니다. 이 경우 전체를 병렬로 전환하는 것이 아니라 스레드 수를 제한(예: 2개)하여 Worker가 수용 가능한 동시 요청 수를 실제 부하 테스트를 통해 확인한 뒤 그에 맞게 조정합니다.
...
**1. 폴링 사이클 내 일시적 불일치**
Worker가 작업을 완료했더라도 다음 폴링 사이클 전까지는 DB가 `PROCESSING`으로 남아 실제와 다른 상태를 응답받을 수 있습니다. 다음 사이클에서 자연 복구됩니다.

**2. 종료 상태 기록 이후 Worker 상태 변경**
